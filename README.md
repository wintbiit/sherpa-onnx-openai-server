...
- `:cpu` is built with this crate's `native-shared` feature and links the official sherpa-onnx Linux x64 shared CPU prebuilt libraries.
- `:cuda` is built with this crate's `native-shared` feature, links the official sherpa-onnx CUDA 12.x + cuDNN 9.x shared GPU prebuilt libraries, and runs on `nvidia/cuda:12.6.3-cudnn-runtime-ubuntu22.04`.
